...
import require$$0, { spawn, execFile } from "child_process";
import require$$1$2, { app, ipcMain, dialog, BrowserWindow } from "electron";
import fs$j from "fs";
  ...
  function remoteTransportFactory(logger) {
    return Object.assign(transport, {
      ...
      sendRequestFn({ serverUrl, requestOptions, body }) {
        const httpTransport = serverUrl.startsWith("https:") ? https$1 : http;
...
const YT_DLP_PATH = require$$1.join(APP_SUPPORT, "yt-dlp");
const FFMPEG_PATH = require$$1.join(APP_SUPPORT, "ffmpeg");
...
const setupYtDlp = async () => {
  ...
  if (fs$j.existsSync(YT_DLP_PATH)) {
    const stats = fs$j.statSync(YT_DLP_PATH);
    if (stats.size > 30 * 1024 * 1024) {
      fs$j.chmodSync(YT_DLP_PATH, "755");
      await new Promise((res) => execFile("xattr", ["-d", "com.apple.quarantine", YT_DLP_PATH], () => res()));
      return;
    }
    fs$j.unlinkSync(YT_DLP_PATH);
  }
...
const setupFFmpeg = async () => {
  ...
  if (fs$j.existsSync(FFMPEG_PATH)) {
    const stats = fs$j.statSync(FFMPEG_PATH);
    if (stats.size > 10 * 1024 * 1024) {
      fs$j.chmodSync(FFMPEG_PATH, "755");
      await new Promise((res) => execFile("xattr", ["-d", "com.apple.quarantine", FFMPEG_PATH], () => res()));
      return;
    }
    fs$j.unlinkSync(FFMPEG_PATH);
  }
  console.log("[Main] Downloading ffmpeg...");
  const zipPath = require$$1.join(APP_SUPPORT, "ffmpeg.zip");
...
function createWindow() {
  ...
  const win = new BrowserWindow({
    ...
    webPreferences: {
      preload: require$$1.join(__dirname$1, "preload.mjs"),
...
app.whenReady().then(async () => {
            ...
            if ((!title || !artist) && url2.includes("soundcloud.com")) {
              try {
                ...
                if (parts.length >= 2) {
                  ...
                  const clean2 = (s) => s.replace(/-/g, " ").replace(/\b\w/g, (l) => l.toUpperCase());
                  if (!title) title = clean2(slugTitle);
                  if (!artist) artist = clean2(slugArtist);
                }
              } catch (e) {
    ...
    return new Promise((resolve) => {
      ...
      child.on("close", (code) => {
        ...
        try {
          ...
          const clean2 = (s) => s.toLowerCase().replace(/[^a-z0-9]/g, "");
          const cleanArtist = clean2(targetArtist);
          const topicMatch = results.find((r) => {
            const uploader = clean2(r.uploader || "");
  ...
  ipcMain.handle("download-song", async (event, { url, folder, artist, title }) => {
    if (initPromise) await initPromise;
    return new Promise((resolve) => {
      const outputTemplate = require$$1.join(folder, `${artist} - ${title}.%(ext)s`);
